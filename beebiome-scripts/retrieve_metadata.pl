...
#This script was generated by Ebot (https://www.ncbi.nlm.nih.gov/Class/PowerTools/eutils/ebot.cgi)
...
# https://www.ncbi.nlm.nih.gov/Class/PowerTools/eutils/course.html
#
#  
# ---------------------------------------------------------------------------
...
foreach my $taxon ( @{ $doc->{TaxaSet}->{Taxon} } ) {
    ...
    my $common_names = $taxon->{OtherNames}->{CommonName};
    if ($common_names) {
        if (ref($common_names) eq 'ARRAY') {
            foreach my $name ( @{$common_names} ) {
                $all_names{$name}++;
            }
        } else {
            $all_names{$common_names}++;
        }
    }

    my $genbank_common_names = $taxon->{OtherNames}->{GenbankCommonName};
    if ($genbank_common_names) {
        if (ref($genbank_common_names) eq 'ARRAY') {
            foreach my $name ( @{$genbank_common_names} ) {
                $all_names{$name}++;
            }
        } else {
            $all_names{$genbank_common_names}++;
        }
    }

    my $synonyms = $taxon->{OtherNames}->{Synonym};
    if ($synonyms) {
        if (ref($synonyms) eq 'ARRAY') {
            foreach my $name ( @{$synonyms} ) {
                $all_names{$name}++;
            }
        } else {
            $all_names{$synonyms}++;
...
if ($params{html} eq 'y') {
  ...
  # print TEMP "$id\t";
   while ($data =~ /<ObjUrl>(.*?)<\/ObjUrl>/sg) {
     ...
     $provurl =~ s/\&amp;/\&/g;
     if ($provurl =~ /^\/entrez/) {
       $provurl = 'https://www.ncbi.nlm.nih.gov' . $provurl;
     }
...
#print HTML
 ...
 while (<IN>) {
   ...
   if ($id ne $cur) {
     ...
     print OUT "<a href=\"https://www.ncbi.nlm.nih.gov/sites/entrez?db=$params{db}&list_uids=$id&cmd=retrieve&dopt=DocSum\" target=\"entrez\">$id</a>&nbsp;&nbsp;";
